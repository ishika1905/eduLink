    ...
    if request.method == 'GET':
        ...
        try:
            ...
            allowed_pictures = [
                ...
                "/Users/angie/StudioProjects/flutter_app/profile_images/pfp.png"  # Default profile picture
            ]

            # Function to validate if the profile picture is in the allowed list or follows a specific pattern
            def is_valid_profile_picture(url):
                # Add custom validation logic here if needed
                return url in allowed_pictures or "/data/Containers/Data/Application/" in url

            # Check if the profile picture is valid
            if is_valid_profile_picture(profile_picture):
                user.profile_picture = profile_picture
                db.session.commit()
                logger.info(f"Profile picture updated successfully for user {current_user_email}")
                return jsonify({'message': 'Profile picture updated successfully'}), 200
            else:
                logger.warning(f"Invalid profile picture selection: {profile_picture}")
